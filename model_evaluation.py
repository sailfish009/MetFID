...
samplelist=[i for i in range(0,26006)]

ttlist=[]
testlist = random.sample(samplelist, 4533)
for i in range(0,26006):
    if i in testlist:
        ttlist.append(1)
    else:
        ttlist.append(0)
...
model = Sequential()
#833
model.add(Dense(800, input_dim=945, activation='relu'))
...
for key,value in test_dict.items():
    ...
    s = s + 1
# -----------output file---------
pickle_out = open("NIST20_LR_predic_dict.p", "wb")
